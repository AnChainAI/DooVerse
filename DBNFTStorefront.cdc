...
//
pub contract DBNFTStorefront {
    // DBNFTStorefrontInitialized
    // This contract has been deployed.
    // Event consumers can now expect events from this contract.
    //
    pub event DBNFTStorefrontInitialized()
    ...
    // DBNFTStorefront workflow, so be careful to check when using them.
    //
    pub event SaleOfferAvailable(
        ...
        ftVaultType: Type,
        price: UFix64,
        sftrxMeta: {String: String}
    )

    // SaleOfferCompleted
    // The sale offer has been resolved. It has either been accepted, or removed and destroyed.
    //
    pub event SaleOfferCompleted(saleOfferResourceID: UInt64, storefrontResourceID: UInt64, accepted: Bool, sftrxMeta: {String: String})
    ...
    pub struct SaleOfferDetails {
        ...
        pub let sftrxMeta: {String: String}
    ...
    pub resource interface SaleOfferPublic {
        ...
        //
        pub fun borrowNFT(): &DBNonFungibleToken.NFT

        // accept
        // Accept the offer, buying the token.
        // This pays the beneficiaries and returns the token to the buyer.
        //
        pub fun accept(payment: @FungibleToken.Vault,trxMeta: {String: String},sftrxMeta: {String: String}): @DBNonFungibleToken.NFT
    ...
    pub resource SaleOffer: SaleOfferPublic {
        ...
        // way that it claims.
        access(contract) let nftProviderCapability: Capability<&{DBNonFungibleToken.Provider, DBNonFungibleToken.CollectionPublic}>
        ...
        pub fun borrowNFT(): &DBNonFungibleToken.NFT {
            ...
            return ref as &DBNonFungibleToken.NFT
        }
        ...
        pub fun accept(payment: @FungibleToken.Vault,trxMeta: {String: String},sftrxMeta: {String: String}): @DBNonFungibleToken.NFT {
            ...
            let nft <-self.nftProviderCapability.borrow()!.withdraw(withdrawID: self.details.nftID,trxMeta:trxMeta)
        ...
        destroy () {
            ...
            if !self.details.accepted {
                emit SaleOfferCompleted(
                    ...
                    storefrontResourceID: self.details.storefrontID,
                    accepted: self.details.accepted,
                    sftrxMeta:self.details.sftrxMeta
                )
            }
        }
        ...
        //
        init (
            nftProviderCapability: Capability<&{DBNonFungibleToken.Provider, DBNonFungibleToken.CollectionPublic}>,
            nftType: Type,
            nftID: UInt64,
            salePaymentVaultType: Type,
            saleCuts: [SaleCut],
    ...
    init () {
        self.StorefrontStoragePath = /storage/DBNFTStorefront
        self.StorefrontPublicPath = /public/DBNFTStorefront

        emit DBNFTStorefrontInitialized()
    }
}